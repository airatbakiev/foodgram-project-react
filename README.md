...
- Ресурс ***recipes***: произведения, к которым пишут отзывы (определённый фильм, книга или песенка).
- Ресурс ***tags***: категории (типы) произведений («Фильмы», «Книги», «Музыка»).
- Ресурс ***ingredients***: жанры произведений. Одно произведение может быть привязано к нескольким жанрам.
...
[http://<ip-адрес сервера>/api/recipes/](http://0.0.0.0/api/recipes/)

***Документация*** (запросы для работы с API):

[http://<ip-адрес сервера>/api/redoc](http://0.0.0.0/api/redoc/)
